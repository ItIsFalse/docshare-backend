...
def get_admin_metrics(
    ...
    return [
        AdminMetric(label="Total Users", value=total_users, change=growth_users),
        AdminMetric(label="Active Doctors", value=active_doctors, change=growth_doctors),
        AdminMetric(label="Pending Appointments", value=pending_appointments, change=growth_appointments),
        AdminMetric(label="Total Patients", value=total_patients, change=15.3)
    ]
...
def get_admin_statistics(
    ...
    for i in range(12):
        ...
        series.append({"period": month, "value": random.randint(100, 200)})

    forecast = []
    for i in range(3):
        month = f"2026-{str(i + 1).zfill(2)}"
        forecast.append({"period": month, "value": random.randint(150, 200)})
...
def get_admin_hospitals(
    ...
    if region:
        query = query.join(Hospital.region).filter(Region.name == region)  # <-- name вместо name_en
    ...
    for hospital in hospitals:
        ...
        region_name = hospital.region.name if hospital.region else "Unknown"  # <-- name вместо name_en

        result.append(AdminHospitalResponse(
            id=hospital.id,
            name=hospital.name_en,  # <-- ИСПРАВЛЕНО
            region=region_name,
            doctors=doctors_count,
            patients=patients_count // 10,
...
def get_admin_doctors(
    ...
    if region:
        query = query.join(Doctor.region).filter(Region.name == region)  # <-- name вместо name_en
    ...
    for doctor in doctors:
        ...
        region_name = doctor.region.name if doctor.region else "Unknown"  # <-- name вместо name_en
        hospital_name = hospital.name_en if hospital else "Unknown"
